...
def calculate_foir(co_applicant_income: float, existing_emi: float, foir_limit: str) -> float:
...
def match_vendors(student_profile: Dict, vendors: List[Dict]) -> Tuple[List[Dict], str]:
    ...
    try:
        logger.info(f"Starting vendor matching for student {student_profile.get('student_id')}")
        ...
        for university in universities:
            supported_vendors = [v['vendorName'] for v in vendors if v.get('active', True)]
            logger.info(f"Found vendors for university {university}: {supported_vendors}")
        ...
        logger.info(f"Filtered to {len(university_vendors)} university-specific vendors")
        
        # Match vendor criteria
        valid_vendors = []
        for vendor in vendors:
            if not vendor.get('active', True) or vendor['vendorName'] not in university_vendors:
                continue
            ...
            existing_emi = co_applicant_details.get('co_applicant_existing_loan_emi_amount', {}).get('amount', 0)
...
def format_vendor_match(vendor: Dict, profile: Dict) -> Dict:
    ...
    return {
        ...
        'max_loan_amount': criteria.get('max_secured_loan_inr' if profile.get('loan_details', {}).get('collateral_available') == 'Yes' else 'max_unsecured_loan_inr'),
        'interest_rate': criteria.get('interest_rate_secured' if profile.get('loan_details', {}).get('collateral_available') == 'Yes' else 'interest_rate_unsecured'),
