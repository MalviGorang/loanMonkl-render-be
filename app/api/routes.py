...
async def create_student(student: Student):
    ...
    try:
        ...
        if student_dict.get("loan_details", {}).get("collateral_available") == "Yes":
            collateral_pincode = student_dict["loan_details"].get("collateral_location_pincode")
            if collateral_pincode and not validate_pincode(collateral_pincode):
                raise HTTPException(status_code=400, detail="Invalid collateral pincode format")
        ...
        if student.current_location_pincode and not validate_pincode(student.current_location_pincode):
            logger.warning(f"Invalid pincode format: {student.current_location_pincode}")
            raise HTTPException(status_code=400, detail="Invalid pincode format")
        if student.loan_details:
            if student.loan_details.cibil_score and not validate_cibil_score(student.loan_details.cibil_score):
                logger.warning(f"Invalid CIBIL score: {student.loan_details.cibil_score}")
            ...
            if student.loan_details.aadhaar and not validate_aadhaar(student.loan_details.aadhaar):
                logger.warning(f"Invalid Aadhaar format: {student.loan_details.aadhaar}")
                raise HTTPException(status_code=400, detail="Invalid Aadhaar format")
            if student.loan_details.co_applicant_pan and not validate_pan(student.loan_details.co_applicant_pan):
                logger.warning(f"Invalid co-applicant PAN format: {student.loan_details.co_applicant_pan}")
                raise HTTPException(status_code=400, detail="Invalid co-applicant PAN format")
            if student.loan_details.co_applicant_aadhaar and not validate_aadhaar(student.loan_details.co_applicant_aadhaar):
                logger.warning(f"Invalid co-applicant Aadhaar format: {student.loan_details.co_applicant_aadhaar}")
        ...
        if student.current_location_pincode:
            ...
            if location:
                ...
                logger.info(f"Pincode {student.current_location_pincode} resolved to city: {location['city']}, state: {location['state']}")
            else:
...
async def get_universities(country: Optional[str] = None, search: Optional[str] = None):
    ...
    logger.info(f"Received GET /api/universities with country: {country}, search: {search}")
    try:
        query = {}
        if country:
        ...
        universities = list(db.universities.find(query, {"name": 1, "vendors": 1, "_id": 0}))
...
async def get_courses(
    ...
    logger.info(f"Received GET /api/courses with course_type: {course_type}, degree: {degree}")
    try:
        query = {}
        ...
        course_names = [course["specialization"] for course in courses if "specialization" in course]
        if not course_names:
...
async def match_vendors(student: Student):
    ...
    logger.info(f"Received POST /api/vendors/match with payload: {json.dumps(student.dict(exclude_unset=True), default=str)}")
    try:
...
async def generate_documents(student: Student):
    ...
    logger.info(f"Received POST /api/documents/generate with payload: {json.dumps(student.dict(exclude_unset=True), default=str)}")
    try:
...
async def get_upload_url(student_id: str, document_type: str, file_name: str):
    ...
    logger.info(f"Received POST /api/documents/upload-url with student_id: {student_id}, document_type: {document_type}, file_name: {file_name}")
    try:
